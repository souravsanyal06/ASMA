...
class ASMA:
    def __init__(self, args):
        rospy.init_node('VLN_ASMA_WD_node')
        ...
        self.obstacle_names = ['bird', 'actor', 'actor2']  
        ...
        safe_distance = self.args['safe_distance'] if 'safe_distance' in self.args else 5.0  # Default to 2.0 if not provided
        self.cbf_controller = self.init_cbf_controller(safe_distance)


    def on_shutdown(self):
        print("Shutdown procedure triggered.")
        np.save(f'drone_path_asma_wd_{self.args.landmark}.npy', self.drone_path)
    ...
    def navigate_to_landmark(self, action, landmark, attribute):
        ...
        if distance < self.close_proximity_threshold + safe_stopping_margin + dimension:
        ...
        obstacles = ['bird', 'actor', 'actor2']  # Example names of moving obstacles
        ...
        for obs_name in obstacles:
            ...
            if obs_distance < self.safe_distance:
                print(f"Close to moving obstacle {obs_name}, applying CBF adjustments.")
                # Include the obstacle in the control calculation
                cbf_control = self.cbf_controller.compute_control_safety_only(drone_pos, selected_pos, [(obs_name, obs_pos)])
                print(f"CBF Control - Linear: {cbf_control.linear.x}, Angular Z: {cbf_control.angular.z}, Angular Y: {cbf_control.angular.y}")

                # Update control message based on CBF output
                control_msg.linear.x = max(control_msg.linear.x, cbf_control.linear.x)  # Ensure some minimum forward velocity
                control_msg.angular.z += cbf_control.angular.z  # Adjust angular velocity as needed
                control_msg.angular.y += cbf_control.angular.y  # Adjust roll as needed
                close_to_obstacle = True
                break

        if not close_to_obstacle:
            print("No close obstacles detected, proceeding with normal navigation.")
