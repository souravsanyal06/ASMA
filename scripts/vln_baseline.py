...
class ASMA:
    def __init__(self, args):
        rospy.init_node('VLN_baseline_node')
    ...
    def compute_mean_depth(self):
        # Assuming depth_array is a 2D numpy array from the depth sensor
        depth_array = self.depth_map
        # import pdb; pdb.set_trace()
        central_region = depth_array[depth_array.shape[0]//2 - 10:depth_array.shape[0]//2 + 10, depth_array.shape[1]//2 - 10:depth_array.shape[1]//2 + 10]
        valid_depths = central_region[np.isfinite(central_region)]
        if valid_depths.size > 0:
            return np.mean(valid_depths)
        return float('inf')  # Return a large number if no valid depths are found
    ...
    def navigate_to_landmark(self, action, landmark, attribute):
        ...
        # self.computeDepth()
        # mean_depth = self.compute_mean_depth()

        # if mean_depth < safe_stopping_margin + dimension:
        #     # import pdb; pdb.set_trace()
        #     print("Close to the landmark based on depth, stopping navigation.")
